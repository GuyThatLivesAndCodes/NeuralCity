...
async fn infer_transformer(
    ...
    tokio::spawn(async move {
        ...
        // Build the inference session once: this uploads the weights to the
        // device and precomputes the RoPE/mask tables a single time, instead
        // of rebuilding the whole module on every generated token.
        let session = {
            let model = model_arc.read().await;
