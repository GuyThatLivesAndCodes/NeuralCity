...
async fn infer_transformer(
    ...
    tokio::spawn(async move {
        ...
        // the RoPE/mask tables on the GPU a single time, then reuse them for
        // every generated token. The previous path re-uploaded the whole model
        // and rebuilt the trig/mask tables on every token. The generation
        // window is always exactly `n_ctx` long (left-padded when shorter), so
        // a single session width covers the whole run.
        let session = {
            let model = model_arc.read().await;
        ...
        for index in 0..max_new {
            ...
            // Logits for the last position only (vocab_size floats).
