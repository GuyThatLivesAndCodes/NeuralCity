...
async fn infer_transformer(
    ...
    tokio::spawn(async move {
        ...
        // The window length is fixed at `n_ctx` (we pad-left when shorter) and
        // the weights are immutable for the whole run, so upload the model and
        // build the RoPE/mask tables exactly once instead of per token.
        let session = {
            let model = model_arc.read().await;
        ...
        for index in 0..max_new {
            ...
            // Logits for the LAST position only — served off the cached session.
            let last_row = session.logits_last(&window);
