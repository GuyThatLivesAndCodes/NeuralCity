...
async fn infer_transformer(
    ...
    tokio::spawn(async move {
        ...
        // Build the inference session once: the weights are static during
        // generation, so we upload them to the GPU a single time and reuse the
        // module (plus its cached RoPE/causal-mask tables) for every token.
        // Previously each token rebuilt the whole module and re-uploaded every
        // weight tensor — the dominant per-token cost.
        let mut session = {
            let model = model_arc.read().await;
        ...
        for index in 0..max_new {
            ...
            // Logits for the last position only (all generation needs).
            let mut row = session.next_logits(&window);
