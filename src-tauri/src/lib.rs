...
async fn infer_transformer(
    ...
    tokio::spawn(async move {
        ...
        // Build the inference session ONCE: upload the weights and precompute
        // the RoPE/causal-mask tensors a single time, then reuse them for every
        // generated token. The previous loop rebuilt the whole GPU module from
        // the CPU weights and refilled the mask on every step. Weights are
        // fixed for the duration of a completion, so the one-time snapshot is
        // also more consistent than re-reading the model lock each token.
        let session = {
            let model = model_arc.read().await;
        ...
        for index in 0..max_new {
            ...
            // Logits for the last position only — that's all generation needs.
            let last_row = session.forward_last(&window);
