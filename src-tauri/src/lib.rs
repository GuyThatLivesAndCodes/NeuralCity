...
async fn infer_transformer(
    ...
    tokio::spawn(async move {
        ...
        // Upload the model to the device and precompute RoPE/mask tables once
        // for the whole generation, instead of rebuilding them per token. The
        // window is always padded to exactly `n_ctx`, so the session's fixed
        // seq_len matches every step.
        let session = {
            let model = model_arc.read().await;
        ...
        for index in 0..max_new {
            ...
            // Forward pass over the fixed window; the session returns only the
            // final-position logits (the next-token distribution).
