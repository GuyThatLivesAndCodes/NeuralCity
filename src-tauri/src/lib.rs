...
async fn infer_transformer(
    ...
    tokio::spawn(async move {
        ...
        // Upload the (frozen) model to the device once and reuse it for every
        // generated token. Rebuilding it per token re-uploaded the whole
        // network and the RoPE/mask tables on each step.
        let mut session = {
            let model = model_arc.read().await;
            neuralcabin_engine::transformer::InferenceSession::<
                neuralcabin_engine::GpuBackend,
            >::new(&model, device.clone())
        };
        ...
        for index in 0..max_new {
            ...
            // forward returns the logits for the LAST position only — sliced
            // on-device so we copy back just `vocab` floats per token.
            let last_row = session.last_logits(&window);
