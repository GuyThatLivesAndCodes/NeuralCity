...
async fn infer_transformer(
    ...
    tokio::spawn(async move {
        ...
        // Build the inference session once: upload weights to the GPU and
        // precompute RoPE/causal-mask tables a single time. The model is
        // immutable during generation and the window is always exactly
        // `n_ctx`, so reusing the session avoids re-uploading every weight and
        // rebuilding the seq_len² mask on each produced token.
        let session = {
            let model = model_arc.read().await;
        ...
        for index in 0..max_new {
            ...
            // The session forward returns logits for the LAST position only.
            let last_row = session.next_logits(&window);
