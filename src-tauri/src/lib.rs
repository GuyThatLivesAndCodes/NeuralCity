...
async fn infer_transformer(
    ...
    tokio::spawn(async move {
        ...
        // Build the inference session once: the model weights live on the GPU
        // and the RoPE/causal-mask tables for the fixed n_ctx window are
        // precomputed, so each generated token only runs the forward pass and
        // pulls back the last position's logits — no per-token weight upload or
        // mask rebuild. The generation window is always exactly n_ctx long
        // (left-padded below), matching the session's fixed seq_len.
        let session = {
            let model = model_arc.read().await;
        ...
        for index in 0..max_new {
            ...
            let mut row = session.logits_last(&window);
