...
async fn infer_transformer(
    ...
    tokio::spawn(async move {
        ...
        // Upload the weights and build the RoPE/mask tables once for the whole
        // run. The window length is always exactly `n_ctx` (left-padded when
        // the context is short), so a single resident session serves every
        // token — turning N per-token weight uploads into one.
        let session = {
            let model = model_arc.read().await;
        ...
        for index in 0..max_new {
            ...
            // Only the last position's logits come back from the device.
            let row_logits = session.logits_last(&window);
            // softmax for sampling.
            let mut row = row_logits;
